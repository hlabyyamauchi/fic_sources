...
#pragma HLS ARRAY_PARTITION variable=buffer cyclic factor=5 dim=1
#pragma HLS ARRAY_PARTITION variable=v cyclic factor=5 dim=2
	 int j, k;
...
void store_output2(float input[MAX_BUF_SIZE], float output[RESULTSIZE]){
#pragma HLS INLINE off
	int i;
...
//timer_out(startt[0], 1.0);//timer start
	if (idd==0) sw2out[0] = 1;
	else sync= sw2in[0];
	...
	//timer_out(stopt[0], 1.0);
	static int stop_flag = 0;
	if (stop_flag == 0) {
		stopt[0] = 1;
		stop_flag = 1;
	}
	//store_output2(buffer, output);
